  ...
  <div class="card-container wide">
    <div class="admin-tabs">
      <button 
        ...
        <span class="material-symbols-outlined icon-fix">group</span> Manage Users
      </button>
      <button 
        class="tab-btn" 
        :class="{ active: currentTab === 'settings' }" 
        @click="currentTab = 'settings'">
        <span class="material-symbols-outlined icon-fix">settings</span> System Settings
      </button>
      <button 
        class="tab-btn" 
        :class="{ active: currentTab === 'register' }" 
        @click="currentTab = 'register'">
        <span class="material-symbols-outlined icon-fix">person_add</span> Register Staff
      </button>
    </div>
    ...
    <div v-if="currentTab === 'manage'" class="tab-content">
      ...
      <div v-if="isLoadingUsers" class="loading-state">
        <span class="material-symbols-outlined rotate-spinner">progress_activity</span> Loading backend staff records...
      </div>
          ...
          <tr v-for="user in filteredStaff" :key="user.employee_id || user.id">
            ...
            <td>
              <button @click="handleDeleteUser(user.employee_id || user.id)" class="action-btn btn-danger">
                <span class="material-symbols-outlined icon-fix">delete</span> Delete
              </button>
            </td>
          </tr>
    ...
    <div v-if="currentTab === 'settings'" class="tab-content">
      <div class="page-header flex-header">
        ...
        <button v-if="!isEditingSettings" @click="isEditingSettings = true" class="action-btn btn-outline-green">
          <span class="material-symbols-outlined icon-fix">edit</span> Edit Mode
        </button>
      </div>
      ...
      <form @submit.prevent="saveSettings" class="settings-form">
        <div class="form-group">
          ...
          <input type="url" v-model="settings.ai_api_url" class="form-control input-no-icon" :disabled="!isEditingSettings" autocomplete="off" required />
        </div>

        <div class="settings-row">
          <div class="form-group half">
            <label>API Timeout Limit (ms)</label>
            <input type="number" v-model="settings.ai_timeout_limit" class="form-control input-no-icon" :disabled="!isEditingSettings" autocomplete="off" required />
          </div>
          <div class="form-group half">
            <label>Scan Interval (min)</label>
            <input type="number" v-model="settings.automatic_scan_interval" class="form-control input-no-icon" :disabled="!isEditingSettings" autocomplete="off" required />
          </div>
        </div>
        ...
        <div class="form-group">
          ...
          <div class="time-grid">
            <div class="time-input-group">
              ...
              <input type="number" v-model="settings.automatic_scan_start_hour" class="form-control input-no-icon" min="0" max="23" :disabled="!isEditingSettings" autocomplete="off" required />
            </div>
            <div class="time-divider">to</div>
            <div class="time-input-group">
              <label class="sub-label">End Hour</label>
              <input type="number" v-model="settings.automatic_scan_end_hour" class="form-control input-no-icon" min="0" max="23" :disabled="!isEditingSettings" autocomplete="off" required />
            </div>
          </div>
        </div>
        ...
        <div class="settings-row">
          <div class="form-group half">
            ...
            <input type="number" step="0.01" min="0" max="1" v-model="settings.aneurysm_high_risk_threshold" class="form-control input-no-icon" :disabled="!isEditingSettings" autocomplete="off" required />
          </div>
          <div class="form-group half">
            <label>Medium Risk Threshold (0.0 - 1.0)</label>
            <input type="number" step="0.01" min="0" max="1" v-model="settings.aneurysm_medium_risk_threshold" class="form-control input-no-icon" :disabled="!isEditingSettings" autocomplete="off" required />
          </div>
        </div>
      ...
      <form @submit.prevent="handleRegister" class="register-form">
        <div class="form-group">
          ...
          <small v-else-if="isEmailFormatValid" class="form-text text-success">
            <span class="material-symbols-outlined icon-fix text-success-icon">check</span> This email address is available.
          </small>
        </div>
...
.admin-tabs { display: flex; gap: 12px; border-bottom: 2px solid #e9ecef; margin-bottom: 30px; }
...
.text-success-icon { font-size: 14px !important; font-weight: bold; vertical-align: middle; margin-right: 2px; }
